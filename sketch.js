...
// Write code to detect collision between player arrow and opponent
 for (var i = 0; i < playerArrows.length; i + 1){
...
function handleComputerArrowCollision() {
  ...
  for (var i = 0; i < computerArrows.length; i + 1){
    var basePCollision = Matter.SAT.collides(
      computerArrows[i].body,
      playerBase.body
    );
 
    var archerPCollision = Matter.SAT.collides(
     computerArrows[i].body,
     playerArcher.body
   );
 
   var pCollision = Matter.SAT.collides(
     computerArrows[i].body,
     player.body
   );
 
   if (basePCollision.collided||archerPCollision.collided||pCollision.collided){
